...
import { Activity, AlertTriangle, BarChart2, FileUp, FolderOpen, ImageIcon, Plus, Send, MessageSquareDashed } from "lucide-react";
...
export default function EmexaAssistant() {
  ...
  const queryProcessed = useRef(false); // Ref lock prevents StrictMode double-executions
  ...
  // Fetch username safely from sessionStorage on mount
  useEffect(() => {
  ...
  // Listen for forwarded parameters from the Report Engine dashboard cards
  useEffect(() => {
    const incomingQuery = searchParams.get("query");
    
    if (incomingQuery && !queryProcessed.current) {
      queryProcessed.current = true; // Lock execution thread

      // Execute auto-submit loop
      handleSendMessage(decodeURIComponent(incomingQuery));

      // Cleanse URL query parameters smoothly without triggering view updates
      ...
      <div className="w-full max-w-2xl backdrop-blur-xl rounded-3xl p-5 space-y-3 mx-auto z-10">
        <form onSubmit={handleFormSubmit} className="relative">
          ...
          <motion.button
            ...
            className="absolute top-7 -translate-y-1/2 left-3 h-10 w-10 text-foreground rounded-full cursor-pointer font-bold flex items-center justify-center transition-all"
          >
            <Plus
              size={20}
        ...
        {messages.length <= 0 && (
          <motion.div
            ...
            initial="hidden"
            animate="show"
            className="flex flex-wrap gap-2 pt-2 border-t justify-center"
          >
            {suggestions.map((suggestion, index) => (
              <motion.button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold rounded-lg dark:bg-[#212121] border border-slate-200 hover:border-slate-700 text-foreground hover:text-primary hover:shadow-md shadow-primary/20 transition-all cursor-pointer flex-col max-w-full text-center whitespace-nowrap"
              >
                {suggestion.icon}
                {suggestion.text}
              </motion.button>
